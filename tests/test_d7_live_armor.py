"""Day-7 live armor window guards (BUILD_PLAN D7-M1 live beat).

Mirrors the Day-5/Day-6 guard suites: the runner refuses without
--confirm-live, refuses under the emulator, and enforces the live-window env
contract (no import-time self-satisfying defaults).
"""
...
from scripts.run_d7_live_armor import main, required_env, validate_live_env


class TestLiveArmorGuards:
    def test_refuses_without_confirm_live_flag(self, capsys: pytest.CaptureFixture[str]) -> None:
        with pytest.raises(SystemExit):
            main(["--deal-id", "deal-redteam-live"])
    ...
    def test_refuses_confirm_live_under_emulator(
        ...
        with pytest.raises(SystemExit):
            main(["--deal-id", "deal-redteam-live", "--confirm-live"])
        captured = capsys.readouterr()
    ...
    def test_refuses_confirm_live_without_env(
        self, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
    ) -> None:
        ...
        with pytest.raises(SystemExit):
            main(["--confirm-live"])
        captured = capsys.readouterr()
        assert "missing live-window env" in captured.err
