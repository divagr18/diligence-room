"""Day-6 live fleet runner guards (small live window, S7).

Mirrors the Day-5 sanity guards: the runner refuses without --confirm-live,
refuses under the emulator, and enforces the Vertex-ADC env contract. The
fleet run itself builds all four deep agents from manifests and drives a real
gemini-3.5-flash agent loop per workstream (Vertex ADC, global location).
"""
...
from scripts.run_d6_live_fleet import main, required_env, validate_live_env


class TestLiveFleetGuards:
